...
التعليمات الصارمة:
1. البوست الأساسي (facebook_post_sales) يجب أن يكون جذاباً، قصير جداً (3 أسطر).
2. إذا كان السعر رقماً واضحاً، اكتبه بأسلوب جذاب وانهي البوست بـ: "لطلب الأوردر ابعتيلنا رسالة 💌 كود الموديل: {code}"
3. إذا كان السعر غير محدد، اكتب: "السعر والتفاصيل في رسايل الصفحة 💌 كود الموديل: {code}"
4. أضف جملتين قصيرتين للريلز.

الرد يجب أن يكون JSON فقط بالهيكل التالي:
{{
  "facebook_post_sales": "نص البوست القصير",
  "story_post": "جملة للستوري",
...
# ==========================================
# الخطة البديلة (تم تعديلها لتصبح طبيعية 100%)
# ==========================================
if not result_json:
    print("💡 تفعيل المحرك التسويقي البديل (نص طبيعي وجذاب).")
    
    price_line = f"💰 السعر: {price} جنيه\n" if price and price != "غير محدد" else ""
    
    fb_post = f"الجمال في التفاصيل ✨\n\nوفرنالك الموديل ده عشان يكمل شياكتك، خامة ممتازة وتقفيل بريميوم 💯\n{price_line}\nلطلب الأوردر ابعتيلنا رسالة للصفحة 💌\nكود الموديل: {code}"
