...
print("⏳ جاري معالجة الصور الفردية وإضافة الختم...")
seen = set()
processed_files = []

# معالجة الصور لإنشاء الألبوم/الكاروسيل (1080x1080)
    ...
    if h not in seen:
        ...
        try:
            ...
            if img_ratio > 1:
                ...
                new_h = 1080
                new_w = int(1080 * img_ratio)
            img_resized = img.resize((new_w, new_h), Image.Resampling.LANCZOS)
            bg.paste(img_resized, ((1080 - new_w) // 2, (1080 - new_h) // 2))
            
            # إضافة الختم / اللوجو في الزاوية السفلية اليسرى
            logo_path = "logo.png"
            if os.path.exists(logo_path):
                ...
                # الإحداثيات للزاوية السفلية اليسرى (مع هامش بسيط)
                x_pos = 30
                y_pos = 1080 - logo_h - 30
                
                # إضافة توهج خلف اللوجو ليكون واضحاً
...
print("🎥 جاري إنشاء فيديو الريلز الحركي من جميع الصور...")
try:
...
try:
    ...
    if result.returncode == 0: video_created = True
except Exception as e: print(f"⚠️ فشل الصوت: {e}")

if not video_created and os.path.exists("reel_video_temp.mp4"): shutil.copy("reel_video_temp.mp4", "reel_video.mp4")
