...
# ==========================================
# إعداد الكولاچ ودمج اللوجو بشكل بارز
# ==========================================
unique_images = []
seen = set()
...
# رسم شريط سفلي داكن عريض جداً (200 بيكسل)
...
if os.path.exists(logo_path):
    ...
    # تكبير اللوجو بشكل ملحوظ جداً (ارتفاع 150 بيكسل)
    logo_target_height = 150 
...
for idx, img_path in enumerate(v_unique):
    try:
        ...
        if os.path.exists(logo_path):
            ...
            logo_y_pos_video = 1920 - reel_footer_h + ((reel_footer_h - logo_reel_h) // 2)
            bg.paste(logo_r, (40, logo_y_pos_video), mask=logo_r)

        if idx == 0: bg = draw_arabic_text(bg, reel_text_1, 250, 85)
        elif idx == len(v_unique) - 1: bg = draw_arabic_text(bg, reel_text_2, 250, 80)
            
        frame = cv2.cvtColor(np.array(bg), cv2.COLOR_RGB2BGR)
        for _ in range(frames_per_img): video.write(frame)
    except: pass
