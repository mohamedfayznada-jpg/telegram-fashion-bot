...
def add_watermark_and_hide_text(image_path, logo_path="logo.png"):
    try:
        base_image = Image.open(image_path).convert("RGBA")
        
        # رسم مربع أسود شفاف لتغطية أي نصوص مقلوبة في الزاوية اليسرى السفلية
        draw = ImageDraw.Draw(base_image)
        margin = 15
        
        # إحداثيات المربع (عرض 160 بكسل، وارتفاع 80 بكسل تقريباً من أسفل اليسار)
        draw.rectangle(
            [0, base_image.height - 90, 170, base_image.height],
            fill=(0, 0, 0, 180) # أسود بشفافية
        )
        
        if not os.path.exists(logo_path):
            print("⚠️ ملف اللوجو (logo.png) غير موجود، سيتم الإكمال بخلفية إخفاء النص فقط.")
        ...
        print(f"❌ خطأ في معالجة اللوجو وإخفاء النص: {e}")
        return image_path
...
story_image = "cover_image.jpg" # الصورة الفردية التي اختارها الذكاء الاصطناعي للغلاف

if os.path.exists(post_file) and os.path.exists(image_file):
    with open(post_file, "r", encoding="utf-8") as f:
        caption = f.read().strip()
        
    print("🎨 جاري معالجة الصورة (إضافة اللوجو وإخفاء الاسم)...")
    final_image_to_post = add_watermark_and_hide_text(image_file)
    ...
    if "id" in res_data:
        ...
        # استدعاء دالة نشر الستوري فوراً
